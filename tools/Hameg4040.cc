...
void Hameg4040::Configure(ChannelMap &fSetChannelMap)
{
	this->Reset();
	write_str = "*IDN?";
...
// 	SystMix();
	...
	fChannelMap=fSetChannelMap;
	for(int i=0;i<4;i++)
	{
		this->WriteChannelNo(i, fChannelMap[i].at(0), fChannelMap[i].at(1));
		this->OutPutSel(i,1);
	}
...
void Hameg4040::MeasAll(ChannelMap& fGetChannelMap)
{
	for(int i=0; i<4; i++)
	{
		this->InstSelect(i);
		this->MeasVolt(fChannelMapCurr[i].at(0));
		this->MeasCurr(fChannelMapCurr[i].at(1));
	}
	fGetChannelMap=fChannelMapCurr;
}

void Hameg4040::GetChannelMap(ChannelMap& fGetChannelMap)
{
	fGetChannelMap=fChannelMapCurr;
}
